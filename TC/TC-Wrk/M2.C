...
/* 4. read switch matrix connection from MACH place/route device file
/*    The input combination is generated from a random number generator.
/*
...
/*     pgmname -d tot_src tot_dest device_filename\n");
/* 	   read connection from MACH device file. inputs are from
...
#define   SUCCESS      1
#define   FAIL         0
#define TOT_RANDNUM 10000
#define MAX_LINE_LENGTH  120


int tot_src, tot_dest, used_dest;   /* number of source and destination */
                         /* command line input */
...
int num_src_conn;         /* number of connections for each source, used for option -r */
...
/**********************************************************************
/*********************************************************************/
void set_up_conn_tbl_from_dev_file(FILE *fp)
{
int i,j,count,dest,scan_val;
char buf[MAX_LINE_LENGTH];

        while(fgets(buf, MAX_LINE_LENGTH, fp)) {
                sscanf(buf, "%d", &scan_val);
                if(scan_val == tot_src) 
                        break;
        }

        for(i=0; i<tot_src; i++) {
                fscanf(fp, "%d%d", &count, &dest);
                for(j=0; j<count; j++) {
                        fscanf(fp , "%d", &dest);
                        *(conn_tbl[i]+dest) = CONNECTED;
                }
                fgets(buf, MAX_LINE_LENGTH, fp);  /* skip the rest of line */
        }
...
void process_input_and_setup(int argc, char *argv[], int *do_comb, int *do_match,
		int *do_generate, int *summary_only, int *do_device,
                int **buf1, int **buf2)
{
int i,j,c,temp;
FILE *fp;
	...
	if(argc < 2) {   
	    ...
	    printf("pgmname -d -s tot_src tot_dest used_dest device_filename\n");
	    printf("    to read connection from device file and use random number\n");
	    printf("    generator to generate inputs\n\n");
            exit(1); 
	}
	...
	if(*do_match || *do_generate || *do_device ) {
	    ...
	    if(*do_device) {
		if(argc != 4) {
                        printf("Invalid input, need tot_src tot_dest used_dest filename\n\n");
                        exit(1); 
                }
		...
		if(*do_match) {
			...
			set_up_conn_tbl_from_dev_file(fp);
		}
	...
	*buf1 = (int *) calloc(used_dest, sizeof(int));
	*buf2 = (int *) calloc(used_dest, sizeof(int));
...
--comb_buf[used_dest-1];  /* decrement the last element by one */
 

/* set up upper bound array */
up_bound[used_dest-1] = tot_src - 1;
for(i=used_dest - 2; i >= 0; i--)
...
/**********************************************************************
/* generate random numbers from 0 to (tot_src - 1)  
/* with no number appearing more than once.
/* There are used_dest number generated.                       
/*********************************************************************/
void generate_random_input(int *randseq) 
{
int i, count = 0, randnum;

	while(count < used_dest) {
        	randnum = rand() % tot_src;
        	for(i=0; i<count; i++)
                	if(randnum == randseq[i])  break;

        	if(i==count)  {  /* a new number not appear before */
                	randseq[count] = randnum;
                	count++;
        	}
	}
}
...
/**********************************************************************
/* check device connection using random number generator to generate
/* inputs
/*********************************************************************/
void process_device_connection(int *unmatch_cnt, int *total_cnt, int summary_only)
{
int i, *randseq;;

	*total_cnt = TOT_RANDNUM;

	/* array to hold random number */
        randseq = (int *) calloc(used_dest, sizeof(int)); 

	srand((unsigned)time(NULL));   /* init rand (random number generator) */

	for(i=0; i<TOT_RANDNUM; i++) {
		generate_random_input(randseq);
                if(!match(randseq)) {  /* cannot route this combination */
                                ++(*unmatch_cnt);
                                if(!summary_only)  print_this_combination(randseq);
                }
	}
}
/**********************************************************************
...
void process_combinations(int do_comb, int do_match, int do_generate, int summary_only,
                  int do_device,int *total_cnt, int *unmatch_cnt, 
                  int comb_buf[], int up_bound[])
{
    if(do_device ) {
	process_device_connection(unmatch_cnt,total_cnt,summary_only);
...
int total_cnt = 0,    /* total number of combinations */
	...
	if(do_generate) { 
		...
		unmatch_cnt = 0;
        	process_combinations(do_comb, do_match, do_generate,  summary_only,
                       do_device,&total_cnt, &unmatch_cnt, comb_buf, 
	     	       up_bound);
	} 
	
 
	printf("\n\n");
	printf(" %d total combinations processed\n",total_cnt);
	printf(" %d tot_source    %d tot_destination    %d used_destination\n",tot_src,tot_dest,used_dest);
	if(!do_comb) {
